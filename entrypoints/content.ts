...
import insertImg from "~/assets/insert.svg";
import generateImg from "~/assets/generate.svg";
import regenerateIcon from "~/assets/regenerate.svg";

export default defineContentScript({
  matches: ["*://*.linkedin.com/*"], // Insert URL use extension on specific web page
  main() {
    // Custom modal
    const modalHtml = `
        ...
        <div id="modal-content" style="background: white; border-radius: 8px; width: 100%; max-width: 570px; padding: 20px;">
          ...
          <div style="text-align: right; margin-top: 12px;">
            <button id="insert-btn" style="background: #fff; color: #666D80; padding: 8px 16px; border: 2px solid #666D80; border-radius: 4px; cursor: pointer; display: none; margin-right: 10px;">
              <img src="${insertImg}" alt="Insert" style="vertical-align: middle; margin-right: 5px; width: 14px; height: 14px;"> 
              <b>Insert</b>
            </button>
            <button id="generate-btn" style="background: #007bff; color: white; padding: 8px 16px; border: 2px solid #007bff; border-radius: 4px; cursor: pointer;">
              <img src="${generateImg}" alt="Generate" style="vertical-align: middle; margin-right: 5px; width: 14px; height: 14px"> 
              <b>Generate</b>
            </button>
          </div>
        </div>
      ...
      if (
        ...
        if (parentElement && !parentElement.querySelector(".edit-icon")) {
          ...
          icon.addEventListener("click", (e) => {
            ...
            // Set focus to the modal input field
            inputText.focus();
          });
        }
      }
    ...
    const generateMessage = () => {
      ...
      return messages[0]; // Dummy message
    };
    ...
    insertBtn.addEventListener("click", () => {
      if (lastGeneratedMessage && parentElement) {
        ...
        if (placeholderDiv) {
          let placeholderText = placeholderDiv.setAttribute('data-placeholder', "");
        }
